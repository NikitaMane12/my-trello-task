...
const Sidebar = () => {
  const blankBoard = {
    name: "",
    bgcolor: "#f60000",
    category: "", // Add a new property for category
    list: [],
  };
  ...
  const filteredBoards = categoryFilter
    ? allboard.boards.filter((board) => board.category === categoryFilter)
    : allboard.boards;

  return (
    <div
      className={`h-[calc(100vh-3rem)] text-2xl border-r text-slate-950 border-r-[#9fadbc29] transition-all linear duration-500 flex-shrink-0 ${
